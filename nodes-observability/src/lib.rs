...
//!   from environment variables into [`TracingConfig`], [`MetricsConfig`] and
//!   related structs.
...
///
/// Typically constructed from environment variables via [`TracingConfig::try_from_env`]
...
///
/// Depending on the [`TracingConfig`]:
///
...
///
/// This helper configures global tracing for the application by:
/// - Building a `tracing_subscriber::fmt::Layer` with targets and line numbers disabled,
///   producing concise logs.
/// - Building an `EnvFilter` from either the `RUST_LOG` environment variable or the provided
///   `env_filter` string as a fallback.
/// - Registering the combined layers into the global `tracing_subscriber::registry`.
///
/// Call this once at startup to initialize tracing before any logs are emitted.
///
/// # Parameters
/// - `env_filter`: Fallback filter string to use if no `RUST_LOG` environment variable is set.
///
/// # Panics
/// Panics if initializing the subscriber fails (this usually happens only if another global
